...
int zerodate = date2int(zerodatestr);
...
static int baddatethreshold = 30;


void oldockodata2R(string input, string output,
                o2rmodes mode,
                unsigned minage, unsigned maxage, unsigned everyn,
                preprocessparams ppp = preprocessparams() ) // rozdelane
{
    ...
    int safetydate = lastdate + baddatethreshold;
    ...
    cout << "Importing from input, total" << data.r() <<
         " records." << endl;
    ...
    cout << "Processing every " << everyn << "-th" << endl;
    cout << "Age filter " << minage << "-" << maxage << endl;
    ...
    unsigned numcensoredbeforestudy = 0;
    unsigned numeventsbeforestudy = 0;
    unsigned numcensoredinstudy = 0;
    unsigned numeventsbeinestudy = 0;
    unsigned numsurvivedstudy = 0;
    ...
    unsigned increment = everyn;

    vector<vector<unsigned>> variants(lastdate-zerodate+1,vector<unsigned>(enumvariants,0));
    for(; i<data.r(); i+= increment)
    {
        if(data(i,DatumOdberu1) != "" && data(i,DatumOdberu2) != "")
            numreinfectionsraw++;
        ...
        if(ddstr != "") // no death
        {
            int ddabs = date2int(ddstr, zerodate, safetydate, ucounter);
            if(ddabs==maxint)
            {
                baddaterecors++;
                continue;
        ...
        if(truedeathbycovdate == maxint)
        {
            ...
            if(dostr != "") // no death
            {
                int doabs = date2int(dostr, zerodate, safetydate, ucounter);
                if(doabs==maxint)
                {
                    baddaterecors++;
                    continue;
        ...
        if(hstr != "") // no hosp
        {
            int habs = date2int(hstr, zerodate ,safetydate, hcounter);
            if(habs==maxint)
            {
                baddaterecors++;
                continue;
            ...
            if(habs <= lastdate)
            {
                int positivity = maxint;
                string pstr = data(i,datum_positivity);
                int pabs = date2int(pstr, zerodate, safetydate, hcounter);
                if(pabs != maxint)
                    positivity = pabs - firstdate;
                hosprecord::econdition oxygen =
                    data(i,ecmo) == "1" || data(i,upv) == "1" || data(i,kyslik) == "1"
        ...
        if(hrstr != "") // no hosp
        {
            int habs = date2int(hrstr, zerodate ,safetydate, hcounter);
            if(habs==maxint)
            {
                baddaterecors++;
                continue;
        ...
        if(okstr != "") // no infection
        {
            ...
            int do1abs;
            if(do1str == "")
                do1abs = date2int(okstr, zerodate, safetydate, do1counter);
            else
                do1abs = date2int(do1str, zerodate, safetydate, do1counter);
            evariant variant1 = eunknownvariant;
            ...
            if(do1abs <= lastdate)
            {
                ...
                infections.push_back({do1abs - firstdate, variant1,s });
                numinfections++;
                variants[do1abs-zerodate][variant1]++;
            }
            ...
            if(do2str != "") // no infection
            {
                int do2abs = date2int(do2str, zerodate, safetydate, do2counter);
                ...
                if(do2abs <= lastdate)
                {
                    ...
                    variants[do2abs-zerodate][variant2]++;
                }
            }
            // tbd third reinfection
        }
        ...
        if(v1str != "")
        {
            ...
            int v1dateabs = date2int(v1datestr, zerodate, safetydate, v1datecounter);
            if(v1dateabs == maxint)
            {
                baddaterecors++;
                continue;
            ...
            vaccrecord v1;
            v1.t = v1dateabs + v1delay - firstdate;
            v1.vaccorder = firstdose;
            v1.vac = v;
            vaccinations.push_back(v1);
            ...
            if(v2str != "")
            {
                ...
                int v2dateabs = date2int(v2datestr, zerodate,safetydate, v2datecounter);
                if(v2dateabs == maxint)
                {
                    baddaterecors++;
                    continue;
                ...
                vaccrecord v2;
                v2.t = v2dateabs + v2delay - firstdate;
                v2.vaccorder = secdose;
                v2.vac = v;
                vaccinations.push_back(v2);
                ...
                if(v3str != "")
                {
                    ...
                    int v3dateabs = date2int(v3datestr, zerodate,safetydate, v3datecounter);
                    if(v3dateabs == maxint)
                    {
                        baddaterecors++;
                        continue;
                    ...
                    vaccrecord v3;
                    v3.t = v3dateabs + v3delay - firstdate;
                    v3.vaccorder = booster;
        ...
        int t1 = zerodate - firstdate; // may be negative, I know, I count wizth it
         ...
         enddate = min(truedeathbycovdate,enddate);

         if(deathbyotherdate < enddate)
             enddate = deathbyotherdate;

         if(mode == ereinfections || mode == evariantreinf || mode == ehospitalsreinf)
         {
             if(vaccinations.size())
             {
                 if(vaccinations[0].t < enddate)
                     enddate = vaccinations[0].t;
             }
         }

         int hospdate = maxint;
         evariant hospvariant = eunknownvariant;
         hosprecord::econdition hospoxygen = hosprecord::enan;
         hosprecord::econdition hospicu = hosprecord::enan;
             ...
             for(unsigned j=0; j<hosps.size(); j++)
             {
                if(ppp.infdateashospdate)
                {
                    if(hosps[j].infdateassigned > 0
                            && hosps[j].infdateassigned != maxint)
                    {
                        hospdate = hosps[j].infdateassigned;
                        hospvariant = hosps[j].variant;
                        hospoxygen = hosps[j].oxygen;
                        hospicu = hosps[j].icu;
                        break;
                    }
                }
         ...
         if((mode == evarianthosp || mode == ehospitals) && hospdate <= enddate)
             enddate = hospdate;
...
// cout << "i=" << i << " t1=" << t1 << endl;
             ...
             for(;nextinfptr < infections.size(); nextinfptr++)
             {
                 if((mode==ehospitals || mode==evarianthosp)
                            && infections[nextinfptr].hospindex != nohosp
                            && infections[nextinfptr].t >= 0) // we have to exclude the infection that cuuset hosp
                     continue;
                 nextinfdate = infections[nextinfptr].t;
                 break;
             }
             ...
             {
                int i = 0;
                for(nextinfstatusupdate=lastinfdate;; nextinfstatusupdate += ppp.reinfduration(i++))
                    if(nextinfstatusupdate > t1)
                        break;
             }
             ...
             if(t2==nextinfdate
                     && (mode == evariantinf || mode == evariantreinf || mode == ecomparison)
                     && t2 > 0)
                 enddate = nextinfdate;
             ...
             int inf;
             int variantinf;
             eseverity newseverity = lastinfseverity;
             if(t2 == nextinfdate)
             {
                 newinfstatus = 1;
                 lastinfdate = t2;
                 ...
                 variantinf = infections[nextinfptr].variant == ppp.variant ? 1 : 0;
                 newseverity = infections[nextinfptr].severity;
                 currentinfection = &(infections[nextinfptr]);
             ...
             if(t2>0)
             {
                 ...
                 string variantoxygenstr = "";
                 string varianticustr = "";
                 string varianthospstr = "";

                 if(mode == ehospitals && t2==hospdate)
                     event = true;
                 ...
                 string severitystr = severitylabel(lastinfseverity);
                 ostringstream os;
                 ...
                 if(mode == ecomparison)
                 {
                     ...
                     if(inf && currentinfection->variant != eunknownvariant)
                     {
                        if(!((ppp.excludeotherimmunity && immunity == otherstr)
                             || (ppp.excludefirstcovreinfduration && currentinfstatus == 1) ))
                        {
                            dooutput = true;
                            string hospstr = "";
                 ...
                 else
                 {
                     ...
                     )
                     {
                         ...
                         os << i << "," << t1nonneg << "," << t2 << ","
                           << inf << ",";
                         if(mode == ehospitals || mode == ehospitalsreinf)
                         {
                             if(t2==hospdate)
                             {
                                 os << "1,";
                                 os << (hospoxygen==hosprecord::eyes ? "1," : "0,");
                                 os << (hospicu==hosprecord::eyes ? "1," : "0,");
                             }
                             else
                                 os << "0,0,0,";
                         }
                         ...
                         string deathstr;
                         string variantdeathstr = "";
                         if((ppp.infdateasdeathdate == false && t2==truedeathbycovdate)
                            || (ppp.infdateasdeathdate == true
                                && deadlyinfection && t2==deadlyinfection->t))
                         {
                             deathstr = "1";
    ...
    cout << processed << " records were used for the analysis" << endl;
    cout << inconsistento << " records is inconsistent"  << endl;
    cout << hospitalunassigned << " records cannot assigm infection to hospitalization"  << endl;
    cout << baddaterecors << " records is with bad or out of range dates" << endl;
    cout << noagerecords << " lacks valid age information " << endl;
    cout << nogenderrecords << " lacks valid genderinformation " << endl;
    cout << deadcovbeforestart << " died of covid before time zero " << endl;
